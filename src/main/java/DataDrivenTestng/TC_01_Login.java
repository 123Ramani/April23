package DataDrivenTestng;
	...
	public class TC_01_Login {
		...
		public Object[][] testData() throws IOException {
			String file=System.getProperty("user.dir")+"/Data/LoginData3.xlsx";
			int rows=Utils.getRowCount(file, "Sheet1");
			int cel=Utils.getCellCount(file, "Sheet1", 0);
			
			Object [][] tdata=new Object[rows][cel];
			for(int i=1;i<=rows;i++) {
				for(int j=0;j<cel;j++) {
					tdata[i-1][j]=Utils.getCellData(file, "Sheet1", i, j);//(0,0)(1,0)
				}
			}
			return tdata;
		}
